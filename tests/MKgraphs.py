...
def test_cli():
    subprocess.run(
        "run-analysis --config tests/MKrunner.json --settings tests/MKsettings.json --fileset tests/NoAEGamma2023_fileset_grouped_shortened.json --binning tests/example_binning.json --output tests/output --executor threads",
        shell=True,
        check=False,
    )

    fileset_path = "tests/NoAEGamma2023_fileset_grouped_shortened.json"
